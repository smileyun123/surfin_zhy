...
# num_records = 40000000  # 插入条数，10万条
base_serial_id = 9000000  # serial_id 起始值
base_task_id = 700000   # task_id 起始值
...
def insert_single_record(conn, cursor, serial_id, tab_name, status, time_value, task_id, update_time, stage):
    """
    ...
    try:
        ...
        # 注意：在多进程中不立即commit，而是在批量操作后再commit
        print("成功准备插入记录")
...
def insert_batch_records(records_data):
    """
    批量插入记录的函数，用于多进程处理

    Args:
        records_data: 包含记录数据的元组列表
    """
    conn, cursor = conn_db()
    try:
        insert_sql = """
        ...
        conn.commit()
        print("批量插入 %s 条记录完成" % len(records_data))
...
def generate_records_batch(args):
    """
    为每个进程生成一批记录数据

    Args:
        args: (batch_size, process_id) 元组

    Returns:
        记录数据列表
    """
    batch_size, process_id = args
    data_batch = []
    # 为每个进程设置不同的随机种子，确保生成的数据有一定差异性
    random.seed(os.getpid() + process_id)

    for i in range(batch_size):
        data_batch.append((
            base_serial_id + random.randint(0, 10000000),  # serial_id
            ...
            base_task_id + random.randint(0, 1000000),  # task_id
...
def multiprocess_insert(total_records=1000000, processes=4):
    """
    使用多进程批量插入数据
    ...
    remaining_records = total_records % processes

    print("开始多进程插入%s 条记录，使用 %s 个进程" % (total_records, processes))

    # 准备每个进程的数据
    process_args = []
    for i in range(processes):
        batch_size = records_per_process + (1 if i < remaining_records else 0)
        process_args.append((batch_size, i))

    # 使用进程池执行多进程插入
    pool = Pool(processes=processes)
    try:
        # 生成数据
        results = pool.map(generate_records_batch, process_args)

        # 插入数据
        insert_results = pool.map(insert_batch_records, results)

        total_inserted = sum(insert_results)
        print("多进程插入完成，总共插入%s 条记录" % total_inserted)
    finally:
        pool.close()
        pool.join()
...
if __name__ == '__main__':
    ...
    # 多进程批量插入示例
    multiprocess_insert(total_records=1000000, processes=4)